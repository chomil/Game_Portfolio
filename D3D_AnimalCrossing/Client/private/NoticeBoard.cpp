...
CNoticeBoard::CNoticeBoard(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

CNoticeBoard::CNoticeBoard(const CNoticeBoard & rhs)
	: CGameObject(rhs)
{
}

HRESULT CNoticeBoard::NativeConstruct_Prototype()
{
...
HRESULT CNoticeBoard::NativeConstruct(void * pArg)
{
...
_int CNoticeBoard::Tick(_double TimeDelta)
{
...
_int CNoticeBoard::Late_Tick(_double TimeDelta)
{
...
HRESULT CNoticeBoard::Render()
{
	...
	if (pPlayer->Get_AnimState() == CPlayer::ANIM_STATE::GET_KEEP)
	{
		...
		if (abs(vPlayerPos.x - (vPos.x+3.5)) < 6 && vPlayerPos.z > vPos.z)
		{
...
HRESULT CNoticeBoard::Render_ShadowDepth()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CNoticeBoard::Add_Components()
{
	/* Com_Renderer */
	...
	/* Com_Mesh */
	if (FAILED(Add_Component(LEVEL_GAMEPLAY0, TEXT("Prototype_Mesh_NoticeBoard"), TEXT("Com_Mesh"), (CComponent**)&m_pMeshCom)))
		return E_FAIL;
	...
	ColliderDesc.vScale = _float3(7.f, 6.0f, 1.0f);
	ColliderDesc.vRadians = _float3(D3DXToRadian(0), 0.0f, 0.f);
	ColliderDesc.vInitPos = _float3(4.0f, 3.0f, -0.5f);
...
HRESULT CNoticeBoard::SetUp_ConstantTable()
{
...
CNoticeBoard * CNoticeBoard::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CNoticeBoard*	pInstance = new CNoticeBoard(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CNoticeBoard");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CNoticeBoard::Clone(void * pArg)
{
	CNoticeBoard*	pInstance = new CNoticeBoard(*this);
