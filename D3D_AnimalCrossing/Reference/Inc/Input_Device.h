...
public:
	...
	{
		return  (m_PrevKeyState[byKey] == 0x00) && (m_KeyState[byKey] & 0x80);
	}
	bool Key_Pressing(_byte byKey)
	{
		return m_KeyState[byKey] & 0x80;
	}
	bool Key_ReleaseOnce(_byte byKey)
	{
		return (m_PrevKeyState[byKey] & 0x80) && (m_KeyState[byKey] == 0x00);
	}
	bool Key_Releasing(_byte byKey)
	{
		return m_KeyState[byKey] == 0x00;
	}
